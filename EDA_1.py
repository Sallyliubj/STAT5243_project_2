...
app_ui = ui.page_fluid(
    ...
    ui.layout_columns(
        ui.value_box(
            "Summary Statistics",
            ui.output_text("summary_statistics_count"),
            showcase=ui.output_text("summary_statistics_type")
        ),
        ui.value_box(
            "Missing Values",
            ui.output_text("missing_values_count"),
            showcase=ui.output_text("missing_values_percent")
        ),
        col_widths=[6, 6]
    ),
...
def server(input, output, session):
    ...
    def data_summary_ui():
        ...
        return ui.TagList(
            ui.h4("Dataset Info"),
            ui.p(f"Rows: {data.shape[0]}, Columns: {data.shape[1]}"),
            ui.p(f"Numeric columns: {data.select_dtypes(include=['number']).columns.size}"),
            ui.p(f"Categorical columns: {data.select_dtypes(include=['object', 'category']).columns.size}")
        )
    ...
    def plot_controls():
        ...
        if plot_type == "histogram":
            ...
            controls.append(ui.input_select("box_y", "Value:", 
                                          {col: col for col in numeric_cols}))
            if len(cat_cols) > 0:
                controls.append(ui.input_select("box_x", "Group by (optional):", 
                                              {"": "None", **{col: col for col in cat_cols}}))
                controls.append(ui.input_select("box_hue", "Color by (optional):", 
                                              {"": "None", **{col: col for col in cat_cols}}))
    ...
    def plot():
        ...
        try:
            if plot_type == "histogram":
                ...
                hue = input.hist_hue() if hasattr(input, "hist_hue") and input.hist_hue() else None
                
                sns.histplot(data=data, x=col, bins=bins, kde=kde, hue=hue)
                ...
                hue = input.scatter_hue() if hasattr(input, "scatter_hue") and input.scatter_hue() else None
                regression = input.scatter_regression() if hasattr(input, "scatter_regression") else False
                
                scatter = sns.scatterplot(data=data, x=x_col, y=y_col, hue=hue)
                ...
                hue = input.bar_hue() if hasattr(input, "bar_hue") and input.bar_hue() else None
                
                if y_col:
                    sns.barplot(data=data, x=x_col, y=y_col, hue=hue)
                    plt.title(f'Bar Plot of {y_col} by {x_col}')
                else:
                    sns.countplot(data=data, x=x_col, hue=hue)
                ...
                y_col = input.box_y() if hasattr(input, "box_y") else data.select_dtypes(include=['number']).columns[0]
                x_col = input.box_x() if hasattr(input, "box_x") and input.box_x() else None
                hue = input.box_hue() if hasattr(input, "box_hue") and input.box_hue() else None
                ...
                sns.violinplot(data=data, x=x_col, y=y_col, hue=hue)
    ...
    def plot_description():
        ...
        try:
            if plot_type == "histogram":
                ...
                description = [
                    ui.h4(f"{plot_type.capitalize()} Plot Analysis: {y_col}"),
                    ui.p(f"Minimum: {data[y_col].min():.4f}"),
                    ui.p(f"Q1 (25%): {data[y_col].quantile(0.25):.4f}"),
                    ui.p(f"Median: {data[y_col].median():.4f}"),
                    ui.p(f"Q3 (75%): {data[y_col].quantile(0.75):.4f}"),
                    ui.p(f"Maximum: {data[y_col].max():.4f}"),
                    ui.p(f"IQR: {(data[y_col].quantile(0.75) - data[y_col].quantile(0.25)):.4f}")
                ]
                
                if x_col:
                    anova = stats.f_oneway(*[group[y_col].values for name, group in data.groupby(x_col)])
                    description.extend([
                        ui.h5(f"Group Comparison ({x_col}):"),
                        ui.p(f"ANOVA F-statistic: {anova.statistic:.4f}"),
                        ui.p(f"ANOVA p-value: {anova.pvalue:.4f}")
                    ])
    ...
    def missing_values_percent():
        ...
        return f"{(missing_cells / total_cells * 100):.2f}% of all cells"
    ...
    def numerical_summary():
        ...
        summary['missing_pct'] = (numeric_data.isna().sum() / len(numeric_data)) * 100
